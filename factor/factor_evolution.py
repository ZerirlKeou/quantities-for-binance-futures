...
def calculate_performance(df, interval, pair, indicator_col='macd_back', periods=None):
    if periods is None:
        periods = [1, 2, 3, 4, 5]
    ...
    for period in periods:
        ...
        long_condition1 = df[df[indicator_col] > 0]
        short_condition1 = df[df[indicator_col] < 0]

        long_wins = df[long_condition][f'Return_{period}'].gt(0).sum()
        short_wins = df[short_condition][f'Return_{period}'].lt(0).sum()
        total_wins = long_wins + short_wins

        long_win_percentage = long_wins / max(len(long_condition1), 1) * 100
        short_win_percentage = short_wins / max(len(short_condition1), 1) * 100
        total_win_percentage = total_wins / len(df) * 100
    ...
    # 将结果追加写入SQLite数据库
    conn = sqlite3.connect(factor_data_path)  # 将 'path' 替换为实际的数据库路径
    results_df.to_sql('performance_results', conn, index=False, if_exists='append')
    conn.close()

def calculate_rate(interval, pair, name):
    """
    Args:
        interval: 时间级别
        pair: 交易对
    ...
    try:
        ...
        calculate_performance(df,interval,pair,indicator_col=name)
    except:
...
def test():
    ...
    for interval in intervals:
        for pair in pairs:
            calculate_rate(interval, pair, 'basic_Open_point')
