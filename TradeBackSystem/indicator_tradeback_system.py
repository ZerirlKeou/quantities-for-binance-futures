            ...
            if times == origin_times:
                ...
                if situation == 'more':
                    ...
                    if df.iloc[i][indicator_name] == -1:
                        ...
                        if times == origin_times:
                            situation = "none"
                    elif self.money[times] + df.iloc[i]['Profit'] <= self.open_money[times] * (1 - stop_loss_percentage):
                        df.iloc[i, profit_column_index] += -self.money[times] * (stop_loss_percentage+self.fee_multiplier)  # 达到止损线卖出
                        times += 1
                        if times == origin_times:
                            situation = "none"
